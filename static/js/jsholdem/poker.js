...
function new_round () {
  ...
  if (num_playing < 2) {
    gui_setup_fold_call_click("Empezar una partida",
                              0,
                              new_game,
                              new_game);
    return;
  ...
  var message = "<tr><td><font size=+2><b>Nueva Ronda</b></font>";
...
function deal_flop () {
  ...
  var message = "<tr><td><font size=+2><b>Flop</b></font>";
  gui_write_game_response(message);
  for (var i = 0; i < 3; i++) {
...
function deal_fourth () {
  ...
  var message = "<tr><td><font size=+2><b>Negociación</b></font>";
  gui_write_game_response(message);
...
function main () {
  ...
  if (players[current_bettor_index].status == "BUST" ||
    ...
    if (current_bettor_index == 0) {
      ...
      if (to_call == 0) {
        call_button_text = "<u>C</u>omprobar";
        fold_button_text = 0;
      ...
      var message = "<tr><td><font size=+2><b>Apuesta actual: " +
                    current_bet_amount +
                    "</b><br> Necesitas <font color=" + hi_lite_color +
                    " size=+3>" + to_call +
                    "</font> más para igualar.</font></td></tr>";
      gui_write_game_response(message);
      write_player(0, 1, 0);
      return;
    } else {
...
function handle_end_of_round () {
  ...
  if (global_pot_remainder) {
    gui_log_to_history("Bote global restante " + global_pot_remainder);
...
//      gui_log_to_history("no winners");
      my_pseudo_alert("No hay ganadores para el bote ");
...
//      gui_log_to_history("no more candidates, pot_remainder " + pot_remainder);
    ...
    gui_log_to_history("Fin de la vuelta");
...
//                     " pot_remainder: " + pot_remainder);
  ...
  for (i = 0; i < allocations.length; i++) {
    if (allocations[i] > 0) {
      ...
      winner_text += winning_hands[i] + " da " + allocations[i] +
                     " a " + players[i].name + ". ";
  ...
  for (i = 0; i < players.length; i++) {
    ...
    detail += players[i].name + " reta " + players[i].total_bet + " & gana " +
              allocations[i] + ".\\n";
  }
  detail = " (<a href='javascript:alert(\"" + detail + "\")'>Detalles</a>)";

  var quit_text = "Empezar de nuevo";
  var quit_func = new_game;
  var continue_text = "Continuar";
  ...
  if (pot_remainder) {
    var local_text = "Hay " + pot_remainder + " puestos en el siguiente bote\n";
    detail += local_text;
  }
  var hi_lite_color = gui_get_theme_mode_highlite_color();
  ...
  if (human_loses == 1) {
    ...
    if (num_playing < 2) {
      ...
      my_pseudo_alert(end_msg + "\n\nEsta partida duró " + elapsed_time + ", " +
                      over_ending);
    }
  }
}
...
function the_bet_function (player_index, bet_amount) {
  if (players[player_index].status == "FOLD") {
    ...
    if (player_index == 0) {
      my_pseudo_alert("La apuesta actual a igualar es " + current_bet_amount +
                      "\nDebes apostar eso " +
    ...
    // COMMENT OUT TO FIND BUGS
    if (player_index == 0) {
      my_pseudo_alert("La mínima subida es " + current_min_raise + ".");
    }
    return 0;
  } else { // RAISE
    players[player_index].status = "CALL";
...
function change_name () {
  var name = prompt("¿Cuál es tu nombre?", getLocalStorage("playername"));
  if (!name) {
    return;
  }
  if (!players) {
    my_pseudo_alert("Demasiado pronto para escoger un nombre");
    return;
  }
  if (name.length > 14) {
    my_pseudo_alert("Demasiado pronto, te voy a llamar Claudia");
    name = "Claudia";
  }
  players[0].name = name;
  write_player(0, 0, 0);
